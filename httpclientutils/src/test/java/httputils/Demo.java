...
import com.hefx.httputils.HttpClientUtil;
import com.hefx.httputils.common.HttpConfig;
import com.hefx.httputils.common.HttpHeader;
import com.hefx.httputils.common.HttpResult;
import com.hefx.httputils.common.SSLs;
import com.hefx.httputils.common.util.HCB;
import com.hefx.httputils.exception.HttpProcessException;
...
public class Demo {
	...
	public static void main(String[] args) throws HttpProcessException, FileNotFoundException {
		String urlPost = "http://api.adf.ly/api.php";
		...
		map.put("key", "youkeyid");
		map.put("youuid", "uid");
		map.put("advert_type", "int");
		map.put("domain", "hefx.com");
		...
		HttpConfig config = HttpConfig.custom()
											...
											;
		HttpConfig configPost = HttpConfig.custom()
				.headers(headers)	//设置headers，不需要时则无需设置
				.timeout(1000) 		//超时
				.url(urlPost)           //设置请求的url
				.map(map)			//设置请求参数，没有则无需设置
				.encoding("utf-8")  //设置请求和返回编码，默认就是Charset.defaultCharset()
				.client(client)     //如果只是简单使用，无需设置，会自动获取默认的一个client对象
				//.inenc("utf-8")   //设置请求编码，如果请求返回一直，不需要再单独设置
		...
		String result1 = HttpClientUtil.get(configPost);    //get请求
		...
		String result2 = HttpClientUtil.post(configPost);   //post请求
